...
#a
ZihuaRongAssignment2$s1a <- nrow(diamonds)
#There are 7 observations 
#b
ZihuaRongAssignment2$s1b <- ncol(diamonds)
#There are 4 columns
#c
ZihuaRongAssignment2$s1c <- names(diamonds)
#The names are "carat" "cut" "clarity" "price"
#d
ZihuaRongAssignment2$s1d <- summary(diamonds$price)
# The result is below
...
NHIS_2007_TSV$weights<- ifelse(test = NHIS_2007_TSV$weight<996 | NHIS_2007_TSV$weight>999,
                               yes = NHIS_2007_TSV$weight,NA)

                               
#f
ZihuaRongAssignment2$s2f <- mean(NHIS_2007_TSV$weights, na.rm=T)
#The new mean weight is 174
#g
ZihuaRongAssignment2$s2g <- median(NHIS_2007_TSV$weights, na.rm=T)
...
#b
ZihuaRongAssignment2$s3b <- paste(vec[c(52,9,8)],collapse="")
#c
...
## Question4
d<-read.dta(file="http://people.ucsc.edu/~aspearot/Econ_217_Data/org_example.dta")
df<-subset(d,select=c(year,month,educ,rw))
ZihuaRongAssignment2$s4<-aggregate(df$rw,list(year=df$year,month=df$month,educ=df$educ),mean,na.rm=T)
